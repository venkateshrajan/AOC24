// AOCD5P1.cpp : Defines the entry point for the application.
//
...
int isCorrectUpdate(const vector<int>& update, map<int, vector<int>>& rules) {
...
97,13,75,29,47)";
	...
	for (const vector<int>& update : updates) {
		answer += isCorrectUpdate(update, rules);
	}
